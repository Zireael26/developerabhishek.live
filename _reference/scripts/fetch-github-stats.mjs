#!/usr/bin/env node
/**
 * Regenerates portfolio/data/stats.json from the GitHub GraphQL API.
 *
...
const OUT = resolve(__dirname, '..', 'portfolio', 'data', 'stats.json');
...
async function repoStats(repo) {
  // commits authored by USERNAME in last 365 days
  ...
  if (!res.ok) {
    // repo might be private or moved; skip quietly
...
(async () => {
  ...
  console.log(`wrote ${OUT}: ${cal.totalContributions} contributions, ${weeks.length} weeks, ${repos.length} repos`);
})();
